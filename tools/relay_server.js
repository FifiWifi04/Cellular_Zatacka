...
const rooms = new Map(); // code -> { code, hostId, peers: Map<id, {ws, name}> }
let nextPeerId = 1;
...
function removePeer(room, id) {
    ...
    if (wasHost) {
        // Host migration is out of scope (T32 territory) -- the round ends.
    ...
    ws.on('message', raw => {
        ...
        if (msg.t === 'create' || msg.t === 'join') {
            ...
            if (msg.t === 'create') {
                ...
                room.peers.set(peerId, { ws, name });
                send(ws, { t: 'joined', room: room.code, id: peerId, host: room.hostId });
            }
            broadcastLobby(room);
            return;
        }
